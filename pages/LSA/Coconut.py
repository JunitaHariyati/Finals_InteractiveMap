...
# -------------------- INITIALIZATION ---------------------
# LOCAL CONSTANT
ASSET_WA   = ASSETS + "LSA_coconut_WA_2025"
ASSET_LF   = ASSETS + "LSA_coconut_LF_2025"
VEGETATION = ASSETS + "classified_2025_2B_ESRI"

_defaults = {
    "coconut_method":        "Weighted Average",
    "coconut_desa":          "Semua Desa",
    "coconut_class":         "Semua Kelas",
    "coconut_clicked_point": None,
    "coconut_click_stats":   None,
}
...
# HEADER
st.title("Analisis Kesesuaian Lahan - Kelapa")
...
with st.container():
    ...
    with fc1:
        ...
        if method != st.session_state.coconut_method:
            st.session_state.coconut_method        = method
            st.session_state.coconut_clicked_point = None
            st.session_state.coconut_click_stats   = None
    ...
    with fc2:
        ...
        sel_desa = st.selectbox(
            "WILAYAH DESA",
            desa_options,
            index=desa_options.index(st.session_state.coconut_desa)
                  if st.session_state.coconut_desa in desa_options else 0,
            key="w_desa",
        )
        if sel_desa != st.session_state.coconut_desa:
            st.session_state.coconut_desa          = sel_desa
            st.session_state.coconut_clicked_point = None
            st.session_state.coconut_click_stats   = None
    ...
    with fc3:
        ...
        sel_class = st.selectbox(
            "KELAS KESESUAIAN",
            class_options,
            index=class_options.index(st.session_state.coconut_class)
                  if st.session_state.coconut_class in class_options else 0,
            key="w_class",
        )
        if sel_class != st.session_state.coconut_class:
            st.session_state.coconut_class = sel_class
...
# -------------------- FILTER CLASS ---------------------
sel_class_val = CLASS_OPTIONS[st.session_state.coconut_class]
...
# -------------------- FILTER DESA ---------------------
if st.session_state.coconut_desa == "Semua Desa":
    ...
    desa_fc         = desaAgats.filter(ee.Filter.eq("NAMOBJ", st.session_state.coconut_desa))
...
with col_map:
        ...
        # LSA LAYER
        Map.addLayer(filtered_image, map_vis, f"LSA - {st.session_state.coconut_class}")
        ...
        if st.session_state.coconut_desa == "Semua Desa":
            ...
            Map.addLayer(
                ...
                {}, f"Desa: {st.session_state.coconut_desa}",
            )
        ...
        _cp = st.session_state.get("coconut_clicked_point")
        ...
        map_key = (
            f"coconut_{st.session_state.coconut_method}_"
            f"{st.session_state.coconut_desa}_"
            f"{st.session_state.coconut_class}"
        )
        ...
        if clicked:
            ...
            if new_pt != st.session_state.coconut_clicked_point:
                st.session_state.coconut_clicked_point = new_pt
                with st.spinner("Mengambil data parameter lokasi…"):
                    try:
                        ...
                        st.session_state.coconut_click_stats = stats
...
with col_stat:
    ...
    with stat_container:
        ...
        # -------------------- PIXEL INFO ---------------------
        cp = st.session_state.coconut_clicked_point
        cs = st.session_state.coconut_click_stats

...
_defaults = {
    "coffee_method":        "Weighted Average",
    "coffee_desa":          "Semua Desa",
    "coffee_class":         "Semua Kelas",
    "coffee_clicked_point": None,
    "coffee_click_stats":   None,
}
...
# HEADER
st.title("Analisis Kesesuaian Lahan - Kopi Liberika")
...
with st.container():
    ...
    with fc1:
        ...
        if method != st.session_state.coffee_method:
            st.session_state.coffee_method        = method
            st.session_state.coffee_clicked_point = None
    ...
    with fc2:
        ...
        if sel_desa != st.session_state.coffee_desa:
            st.session_state.coffee_desa          = sel_desa
            st.session_state.coffee_clicked_point = None
            st.session_state.coffee_click_stats   = None
    ...
    with fc3:
        ...
        if sel_class != st.session_state.coffee_class:
            st.session_state.coffee_class = sel_class
...
if st.session_state.coffee_desa == "Semua Desa":
    ...
    desa_fc         = desaAgats.filter(ee.Filter.eq("NAMOBJ", st.session_state.coffee_desa))
...
with col_map:
        ...
        # LSA LAYER
        Map.addLayer(filtered_image, map_vis, f"LSA - {st.session_state.coffee_class}")
        ...
        if st.session_state.coffee_desa == "Semua Desa":
            ...
            Map.addLayer(
                ...
                {}, f"Desa: {st.session_state.coffee_desa}",
            )
        ...
        _cp = st.session_state.get("coffee_clicked_point")
        ...
        map_key = (
            f"coffee_{st.session_state.coffee_method}_"
            f"{st.session_state.coffee_desa}_"
            f"{st.session_state.coffee_class}"
        )
        ...
        if clicked:
            ...
            if new_pt != st.session_state.coffee_clicked_point:
                st.session_state.coffee_clicked_point = new_pt
                with st.spinner("Mengambil data parameter lokasi…"):
                    try:
                        ...
                        st.session_state.coffee_click_stats = stats

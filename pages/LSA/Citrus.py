...
# -------------------- INITIALIZATION ---------------------
# LOCAL CONSTANT
ASSET_WA   = ASSETS + "LSA_citrus_WA_2025"
ASSET_LF   = ASSETS + "LSA_citrus_LF_2025"
VEGETATION = ASSETS + "classified_2025_2B_ESRI"

_defaults = {
    "citrus_method":        "Weighted Average",
    "citrus_desa":          "Semua Desa",
    "citrus_class":         "Semua Kelas",
    "citrus_clicked_point": None,
    "citrus_click_stats":   None,
}
...
# HEADER
st.title("Analisis Kesesuaian Lahan - Jeruk")
...
with st.container():
    ...
    with fc1:
        ...
        if method != st.session_state.citrus_method:
            st.session_state.citrus_method        = method
            st.session_state.citrus_clicked_point = None
            st.session_state.citrus_click_stats   = None
    ...
    with fc2:
        ...
        sel_desa = st.selectbox(
            "Wilayah Desa",
            desa_options,
            index=desa_options.index(st.session_state.citrus_desa)
                  if st.session_state.citrus_desa in desa_options else 0,
            key="w_desa",
        )
        if sel_desa != st.session_state.citrus_desa:
            st.session_state.citrus_desa          = sel_desa
            st.session_state.citrus_clicked_point = None
            st.session_state.citrus_click_stats   = None
    ...
    with fc3:
        ...
        sel_class = st.selectbox(
            "Kelas Kesesuaian",
            class_options,
            index=class_options.index(st.session_state.citrus_class)
                  if st.session_state.citrus_class in class_options else 0,
            key="w_class",
        )
        if sel_class != st.session_state.citrus_class:
            st.session_state.citrus_class = sel_class
...
# -------------------- FILTER DESA ---------------------
if st.session_state.citrus_desa == "Semua Desa":
    ...
    desa_fc         = desaAgats.filter(ee.Filter.eq("NAMOBJ", st.session_state.citrus_desa))
...
with st.spinner("Menghitung statistik luas…"):
    df_area = calculate_area_stats(
        filtered_image,
        region_geometry,
        region_key=st.session_state.citrus_desa,
        method_key=st.session_state.citrus_method,
    )
...
with col_map:
        ...
        # LSA LAYER
        Map.addLayer(filtered_image, map_vis, f"LSA - {st.session_state.citrus_class}")
        ...
        if st.session_state.citrus_desa == "Semua Desa":
            ...
            Map.addLayer(
                ...
                {}, f"Desa: {st.session_state.citrus_desa}",
            )
        ...
        _cp = st.session_state.get("citrus_clicked_point")
        ...
        map_key = (
            f"citrus_{st.session_state.citrus_method}_"
            f"{st.session_state.citrus_desa}_"
            f"{st.session_state.citrus_class}"
        )
        ...
        if clicked:
            ...
            if new_pt != st.session_state.citrus_clicked_point:
                st.session_state.citrus_clicked_point = new_pt
                with st.spinner("Mengambil data parameter lokasi…"):
                    try:
                        ...
                        st.session_state.citrus_click_stats = stats
...
with col_stat:
    ...
    with stat_container:
        ...
        # -------------------- PIXEL INFO ---------------------
        cp = st.session_state.citrus_clicked_point
        cs = st.session_state.citrus_click_stats

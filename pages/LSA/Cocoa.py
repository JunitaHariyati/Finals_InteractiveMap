...
# -------------------- INITIALIZATION ---------------------
# LOCAL CONSTANT
ASSET_WA   = ASSETS + "LSA_cocoa_WA_2025"
ASSET_LF   = ASSETS + "LSA_cocoa_LF_2025"
VEGETATION = ASSETS + "classified_2025_2B_ESRI"

_defaults = {
    "cocoa_method":        "Weighted Average",
    "cocoa_desa":          "Semua Desa",
    "cocoa_class":         "Semua Kelas",
    "cocoa_clicked_point": None,
    "cocoa_click_stats":   None,
}
...
with st.container():
    ...
    with fc1:
        ...
        if method != st.session_state.cocoa_method:
            st.session_state.cocoa_method        = method
            st.session_state.cocoa_clicked_point = None
            st.session_state.cocoa_click_stats   = None
    ...
    with fc2:
        ...
        sel_desa = st.selectbox(
            "WILAYAH DESA",
            desa_options,
            index=desa_options.index(st.session_state.cocoa_desa)
                  if st.session_state.cocoa_desa in desa_options else 0,
            key="w_desa",
        )
        if sel_desa != st.session_state.cocoa_desa:
            st.session_state.cocoa_desa          = sel_desa
            st.session_state.cocoa_clicked_point = None
            st.session_state.cocoa_click_stats   = None
    ...
    with fc3:
        ...
        sel_class = st.selectbox(
            "KELAS KESESUAIAN",
            class_options,
            index=class_options.index(st.session_state.cocoa_class)
                  if st.session_state.cocoa_class in class_options else 0,
            key="w_class",
        )
        if sel_class != st.session_state.cocoa_class:
            st.session_state.cocoa_class = sel_class
...
# -------------------- FILTER CLASS ---------------------
sel_class_val = CLASS_OPTIONS[st.session_state.cocoa_class]
...
# -------------------- FILTER DESA ---------------------
if st.session_state.cocoa_desa == "Semua Desa":
    ...
    desa_fc         = desaAgats.filter(ee.Filter.eq("NAMOBJ", st.session_state.cocoa_desa))
...
with col_map:
        ...
        # LSA LAYER
        Map.addLayer(filtered_image, map_vis, f"LSA - {st.session_state.cocoa_class}")
        ...
        if st.session_state.cocoa_desa == "Semua Desa":
            ...
            Map.addLayer(
                ...
                {}, f"Desa: {st.session_state.cocoa_desa}",
            )
        ...
        _cp = st.session_state.get("cocoa_clicked_point")
        ...
        map_key = (
            f"cocoa_{st.session_state.cocoa_method}_"
            f"{st.session_state.cocoa_desa}_"
            f"{st.session_state.cocoa_class}"
        )
        ...
        if clicked:
            ...
            if new_pt != st.session_state.cocoa_clicked_point:
                st.session_state.cocoa_clicked_point = new_pt
                with st.spinner("Mengambil data parameter lokasi…"):
                    try:
                        ...
                        st.session_state.cocoa_click_stats = stats
...
with col_stat:
    ...
    with stat_container:
        ...
        # -------------------- PIXEL INFO ---------------------
        cp = st.session_state.cocoa_clicked_point
        cs = st.session_state.cocoa_click_stats

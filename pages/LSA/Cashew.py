...
# -------------------- INITIALIZATION ---------------------
# LOCAL CONSTANT
ASSET_WA   = ASSETS + "LSA_cashew_WA_2025"
ASSET_LF   = ASSETS + "LSA_cashew_LF_2025"
VEGETATION = ASSETS + "classified_2025_2B_ESRI"

_defaults = {
    "cashew_method":        "Weighted Average",
    "cashew_desa":          "Semua Desa",
    "cashew_class":         "Semua Kelas",
    "cashew_clicked_point": None,
    "cashew_click_stats":   None,
}
...
# HEADER
st.title("Analisis Kesesuaian Lahan - Kacang Mete")
...
with st.container():
    ...
    with fc1:
        ...
        if method != st.session_state.cashew_method:
            st.session_state.cashew_method        = method
            st.session_state.cashew_clicked_point = None
            st.session_state.cashew_click_stats   = None
    ...
    with fc2:
        ...
        sel_desa = st.selectbox(
            "Wilayah Desa",
            desa_options,
            index=desa_options.index(st.session_state.cashew_desa)
                  if st.session_state.cashew_desa in desa_options else 0,
            key="w_desa",
        )
        if sel_desa != st.session_state.cashew_desa:
            st.session_state.cashew_desa          = sel_desa
            st.session_state.cashew_clicked_point = None
            st.session_state.cashew_click_stats   = None
    ...
    with fc3:
        ...
        sel_class = st.selectbox(
            "Kelas Kesesuaian",
            class_options,
            index=class_options.index(st.session_state.cashew_class)
                  if st.session_state.cashew_class in class_options else 0,
            key="w_class",
        )
        if sel_class != st.session_state.cashew_class:
            st.session_state.cashew_class = sel_class
...
# -------------------- FILTER CLASS ---------------------
sel_class_val = CLASS_OPTIONS[st.session_state.cashew_class]
...
# -------------------- FILTER DESA ---------------------
if st.session_state.cashew_desa == "Semua Desa":
    ...
    desa_fc         = desaAgats.filter(ee.Filter.eq("NAMOBJ", st.session_state.cashew_desa))
...
with st.spinner("Menghitung statistik luas…"):
    df_area = calculate_area_stats(
        filtered_image,
        region_geometry,
        region_key=st.session_state.cashew_desa,
        method_key=st.session_state.cashew_method,
    )
...
with col_map:
        ...
        # LSA LAYER
        Map.addLayer(filtered_image, map_vis, f"LSA - {st.session_state.cashew_class}")
        ...
        if st.session_state.cashew_desa == "Semua Desa":
            ...
            Map.addLayer(
                ...
                {}, f"Desa: {st.session_state.cashew_desa}",
            )
        ...
        _cp = st.session_state.get("cashew_clicked_point")
        ...
        map_key = (
            f"cashew_{st.session_state.cashew_method}_"
            f"{st.session_state.cashew_desa}_"
            f"{st.session_state.cashew_class}"
        )
        ...
        if clicked:
            ...
            if new_pt != st.session_state.cashew_clicked_point:
                st.session_state.cashew_clicked_point = new_pt
                with st.spinner("Mengambil data parameter lokasi…"):
                    try:
                        ...
                        st.session_state.cashew_click_stats = stats
...
with col_stat:
    ...
    with stat_container:
        ...
        # -------------------- PIXEL INFO ---------------------
        cp = st.session_state.cashew_clicked_point
        cs = st.session_state.cashew_click_stats
